 ...
 * Звезда — раздел «Дуа».
 *
 * Владелец 10.09.2026 попросил звезду. В паре с полумесяцем азкаров это
 * читается с одного взгляда и без объяснений, чего прежним значкам не
 * удавалось: ладони на 25 px перепробовали четырьмя способами и все четыре
 * читались чужим — короной, якорем, улыбающимся лицом, бутоном.
 *
 * Ближайший системный аналог — SF Symbols `star` / `star.fill`, пятиконечная
 * с чуть скруглёнными вершинами. Геометрия классическая: внешние вершины на
 * окружности радиуса 8.8, внутренние — на 3.6, поворот вершиной вверх.
 *
 * Имя прежнее было `Flower` — историческое, значок переезжал от цветка к
 * ладони и обратно. Переименовано сейчас, потому что значок остался всего в
 * одном месте (панель вкладок) и прежнее возражение «задело бы полтора
 * десятка импортов» отпало.
 */
export const TabDua = ({ size = ICON_SIZE.md, isFilled = false, className, style }: SelectableProps) => (
  <svg {...stroke(size, className, style)}>
    <path
      d="M12 3.25 14.72 8.76 20.8 9.65 16.4 13.93 17.44 19.99 12 17.13 6.56 19.99 7.6 13.93 3.2 9.65 9.28 8.76Z"
