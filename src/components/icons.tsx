...
/**
 * Цветок — иконка раздела «Дуа».
 *
 * Раньше там стояла закладка, но она уже занята: тем же значком
 * помечены закладки Корана, и в одном ряду это читалось как «то же
 * самое».  Цветок предложил владелец, и он подходит: шесть лепестков
 * вокруг центра — узнаваемый мотив исламского орнамента, тот же, что в
 * розетке над номером аята.
 *
 * Лепестки заданы одним path и повёрнуты: шесть отдельных эллипсов в
 * 21 px слились бы в пятно, а капля с заострённым концом держит форму.
 */
export const Flower = ({ size = 20 }: Props) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
...
/** Восход — утренние азкары.  Полусолнце над линией горизонта плюс
 *  короткие лучи: на 18 px читается именно как «утро», а не как
 *  абстрактный круг. */
export const Sunrise = ({ size = 18 }: Props) => (
  <svg {...stroke(size)}>
    <path d="M4.5 17.5h15" />
    <path d="M7.75 17.5a4.25 4.25 0 0 1 8.5 0" />
    <path d="M12 4.4v2.3" />
    <path d="m6.4 6.9 1.6 1.6" />
    <path d="m17.6 6.9-1.6 1.6" />
  </svg>
);

/** Закат — вечерние азкары.  То же полусолнце, но лучи направлены
 *  вниз: пара «вверх / вниз» различима с одного взгляда, а два разных
 *  символа пришлось бы читать. */
export const Sunset = ({ size = 18 }: Props) => (
  <svg {...stroke(size)}>
    <path d="M4.5 17.5h15" />
    <path d="M7.75 17.5a4.25 4.25 0 0 1 8.5 0" />
    <path d="M12 8.2V5.9" />
    <path d="m9.4 6.5 2.6 2.6 2.6-2.6" />
  </svg>
);
