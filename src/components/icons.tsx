...
export const TabAzkar = ({ size = ICON_SIZE.md, isFilled = false, className, style }: SelectableProps) => (
  <svg {...stroke(size, className, style)}>
    ...
    {/* Связующей дуги нет намеренно: с ней бусины слипались в подвеску
        (снимок 08.09.2026). Нить читается по самому ряду бусин. */}
    {([[5.2, 7.4], [6.4, 11.0], [8.9, 13.6], [12, 14.5], [15.1, 13.6], [17.6, 11.0], [18.8, 7.4]] as const).map(([cx, cy]) => (
      <circle key={`${cx}-${cy}`} cx={cx} cy={cy} r="1.32"
        fill={isFilled ? 'currentColor' : 'none'} />
    ))}
    <circle cx="12" cy="18.6" r="1.5" fill={isFilled ? 'currentColor' : 'none'} />
    <path d="M12 20.3v1.2" fill="none" />
  </svg>
);
