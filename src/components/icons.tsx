 ...
 * Сердце — раздел «Дуа».
 *
 * Владелец 14.09.2026 выбрал сердце: дуа — обращение от сердца. До этого
 * были цветок, ладонь и звезда (10.09.2026). Ладони на 25 px перепробовали
 * четырьмя способами, и все четыре читались чужим — короной, якорем,
 * улыбающимся лицом, бутоном; к ним не возвращаться без замера на экране.
 *
 * Ближайший системный аналог — SF Symbols `heart` / `heart.fill`. Две доли —
 * кубические дуги, сходящиеся в выемке (12, 7.1) и в остром низе (12, 19.4);
 * габарит 4…20 по ширине и 4.8…19.4 по высоте — оптический центр чуть выше
 * геометрического, как у полумесяца и часов рядом. Имя `TabDua` сохранено:
 * значок живёт в одном месте, в панели вкладок.
 */
export const TabDua = ({ size = ICON_SIZE.md, isFilled = false, className, style }: SelectableProps) => (
  <svg {...stroke(size, className, style)}>
    <path
      d="M12 19.4C12 19.4 4 14.55 4 9.1C4 6.6 5.9 4.8 8.2 4.8C9.8 4.8 11.2 5.7 12 7.1C12.8 5.7 14.2 4.8 15.8 4.8C18.1 4.8 20 6.6 20 9.1C20 14.55 12 19.4 12 19.4Z"
