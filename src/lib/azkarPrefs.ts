 ...
 *  • `source` — по полю `page`: `Page1`, `Page1_2`, `Page1_3`, `Page2` … и в
 *    конце `Page_Kursi` и `Page_Baqara285`. Это страницы исходной книги, то
 *    есть её собственная последовательность: сперва три суры, потом остальное.
 *
 * 🔴 Почему именно `page`, а не порядок записей в массиве.
 *
 * Сперва `source` был сделан как «порядок записей в файле», и для УТРЕННИХ это
 * одно и то же — там массив идёт ровно по страницам. А у вечерних массив
 * отсортирован по `id` и по страницам скачет: 1, 2, 3, 7, 9, 10, 11, 14, 13,
 * 8, 6, 4, 5. То есть «как изначально» у вечерних получалось не как в книге, а
 * как удобно было выгружать. `page` даёт ровный ход 1…14 в обеих категориях —
 * это и есть исходная последовательность.
 *
 * ⚠️ Что НЕ является этими двумя порядками: книги записей. У зикров есть
 * аудио из двух книг (`audio_book1` / `audio_book2`), но относительная
 * последовательность у них одинаковая — «Книга 1 против Книги 2» порядка не
 * задаёт. Проверено на данных.
 *
 ...
 */
export function orderAzkar<T extends { n_in_category?: number; page?: string }>(
  список: readonly T[],
  order: AzkarOrder,
): T[] {
  if (order !== 'source') {
  ...
  // Страницы вида `Page3` и `Page1_2` (вторая часть той же страницы).
  // Именованные — `Page_Kursi`, `Page_Baqara285` — идут в конец, сохраняя
  // свой относительный порядок: сортировка в JS устойчива.
  const вес = (x: T): [number, number, number] => {
    const m = /^Page(\d+)(?:_(\d+))?$/.exec(x.page ?? '');
    return m ? [0, Number(m[1]), Number(m[2] ?? 0)] : [1, 0, 0];
  };
  return [...список].sort((a, b) => {
    const [ga, pa, sa] = вес(a);
    const [gb, pb, sb] = вес(b);
    return ga - gb || pa - pb || sa - sb;
  });
}
