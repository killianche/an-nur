#!/usr/bin/env python3
"""
add-svg-in-ot.py — embed an SVG-in-OpenType table next to COLR v1.
...
coloured glyph, mirroring the COLR paint tree but with the dark
palette[0] colours baked in.
...
Cost / trade-offs:
  ...
  * Palette becomes BAKED — overriding colours via
    @font-palette-values has no effect on the SVG render path.  On
    iOS that means the user sees the dark palette[0] (patched in by
    patch-tajweed-default-palette.py) and cannot toggle individual
    rules.  On Chrome desktop, @font-palette-values still works
    through the COLR path.

Idempotent: if an SVG table is already present and its glyph count
matches the COLR v1 BaseGlyphList, the font is skipped.

Pipeline order: run LAST, after upgrade-tajweed-colr-v1.py.

Usage:
  python3 scripts/add-svg-in-ot.py
  python3 scripts/add-svg-in-ot.py p001
"""
...
FOREGROUND_PALETTE_INDEX = 0xFFFF  # special — render with `currentColor`
...
def paint_solid_color(paint, cpal) -> str:
    """PaintSolid → SVG fill string.  Foreground (0xFFFF) → currentColor."""
    idx = paint.PaletteIndex
    if idx == FOREGROUND_PALETTE_INDEX:
        return "currentColor"
...
def process_one(path: Path) -> str:
    ...
    if "SVG " in font:
        existing = font["SVG "]
        # quick freshness check: same number of documents as base glyphs
        if len(existing.docList) == len(base_records):
