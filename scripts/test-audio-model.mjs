...
await groupAsync('Мусхаф: лист летит по физике, а не по переходу', async () => {
  ...
  check('за спокойное время (0.6–1.1 с)', спокойно.мс > 600 && спокойно.мс < 1100, true);

  // 🔴 Главное обещание владельцу: бросок листается быстрее спокойного жеста.
  const бросок = прогон(300, 2.5, ШАГ);
  ...
  check('сверхсильный бросок упирается в потолок скорости, а не в TAU_MIN',
    approachTau(300, 99), 300 / MAX_SPEED);
  ...
  // 300 на 1.5 — это 200 мс, выше потолка: берётся потолок.
  check('слишком вялый бросок не растягивает полёт', approachTau(300, 1.5), TAU_MAX);
