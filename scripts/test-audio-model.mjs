...
await groupAsync('Мусхаф: лист летит по физике, а не по переходу', async () => {
  const mod = await import(
    pathToFileURL(resolve(ROOT, 'src/lib/mushafFling.ts')).href
  );
  const {
    projectTarget, stepFling, isSettled, clampSpeed, approachTau,
    TAU_MIN, TAU_MAX, MAX_SPEED,
  } = mod;
  ...
  // ── Как он туда едет ───────────────────────────────────────────────
  const прогон = (x0, v0, target) => {
    const tau = approachTau(x0 - target, v0);
    let s = { x: x0, v: v0 }, t = 0, перелёт = 0;
    const сторона = Math.sign(x0 - target);
    for (let i = 0; i < 900; i++) {
      s = stepFling(s, target, 16.7, tau);
      t += 16.7;
      if (Math.sign(s.x - target) === -сторона && Math.abs(s.x - target) > 0.5) перелёт = 1;
      if (isSettled(s, target)) break;
    }
    return { мс: Math.round(t), перелёт, x: +s.x.toFixed(2) };
  };

  const спокойно = прогон(ШАГ * 0.75, 0, 0);
  check('спокойно отпущенный лист приходит на место', спокойно.x < 0.5, true);
  check('и делает это без перелёта', спокойно.перелёт, 0);
  // Ожидание выводится из самой константы, а не из числа в тесте: ручку
  // «медленнее/быстрее» владелец двигает, и тест обязан проверять закон,
  // а не прошлое значение. Весь путь до полупикселя — τ·ln(путь/0.5).
  const ждёмСпокойно = TAU_MAX * Math.log((ШАГ * 0.75) / 0.5);
  check('спокойное возвращение идёт столько, сколько задаёт TAU_MAX (±20 %)',
    Math.abs(спокойно.мс - ждёмСпокойно) < ждёмСпокойно * 0.2, true);

  // 🔴 Главное обещание владельцу: бросок листается быстрее спокойного жеста.
  const бросок = прогон(300, MAX_SPEED, ШАГ);
  const вяло   = прогон(300, 0,   ШАГ);
  check('брошенный лист приходит быстрее спокойного', бросок.мс < вяло.мс, true);
  check('заметно быстрее, а не на пару кадров', бросок.мс * 2 < вяло.мс, true);
  // 🔴 Перелёта нет ни при какой скорости: бумага доходит до места и стоит,
  // а не отскакивает резинкой. Пружина в критическом затухании этим свойством
  // НЕ обладает — на сильном броске она цель проскакивала.
  check('и без перелёта даже на сильном броске', бросок.перелёт, 0);
  for (const v of [0.5, 1, 2, 3, MAX_SPEED]) {
    check(`перелёта нет при скорости ${v}`, прогон(300, v, ШАГ).перелёт, 0);
  }

  // Пропущенные кадры не должны разгонять движение.
  let рваный = { x: ШАГ, v: 0 };
  for (let i = 0; i < 40; i++) рваный = stepFling(рваный, 0, 400);
  check('пропущенные кадры не разгоняют лист', Math.abs(рваный.x) < 1, true);

  // ── Постоянная времени ─────────────────────────────────────────────
  check('без скорости берётся самое спокойное время', approachTau(300, 0), TAU_MAX);
  // 🔴 Потолок скорости и нижняя граница времени связаны: при MAX_SPEED
  // 4.5 px/мс путь в 300 px быстрее 66.7 мс не пройти вообще, сколько ни
  // бросай. Нижняя граница TAU_MIN работает на коротких остатках.
  check('сверхсильный бросок упирается в потолок скорости или в TAU_MIN',
    approachTau(300, 99), Math.max(TAU_MIN, 300 / MAX_SPEED));
  check('на коротком остатке работает нижняя граница времени',
    approachTau(150, 99), TAU_MIN);
  // 300 px на скорости 3 px/мс — это 100 мс, между границами.
  check('средний бросок даёт время между границами', approachTau(300, 3), 100);
  // 300 на 1 px/мс — это 300 мс, выше потолка: берётся потолок.
  check('слишком вялый бросок не растягивает полёт', approachTau(300, 1), TAU_MAX);
  check('нулевой остаток не делит на ноль', approachTau(0, 0), TAU_MIN);

  // Скорость ограничена: бросок продолжает жест, а не телепортирует.
  ...
  // Ключевые кадры обязаны описывать ту же траекторию, что и формула:
  // по ней считается положение при перехвате касанием.
  const { flightAt, flightKeyframes, flightDuration, FLIGHT_SAMPLES } = mod;
  const полёт = flightKeyframes(300, 0, TAU_MAX);
  check('кадры полёта начинаются там, где лист', Math.abs(полёт.xs[0] - 300) < 1e-9, true);
  check('и кончаются ровно в цели', полёт.xs[полёт.xs.length - 1], 0);
  check('кадров столько, сколько задано', полёт.xs.length, FLIGHT_SAMPLES + 1);
  check('длительность — до полупикселя от цели',
    полёт.duration, flightDuration(300, TAU_MAX));
  check('без перелёта: все кадры между началом и целью',
    полёт.xs.every(x => x >= 0 && x <= 300), true);
  check('кадры монотонны',
    полёт.xs.every((x, i) => i === 0 || x <= полёт.xs[i - 1]), true);
  check('на исходе полёта лист в полупикселе от цели',
    Math.abs(flightAt(300, 0, TAU_MAX, полёт.duration).x) <= 0.5, true);
  // Формула от начала полёта совпадает с пошаговой — иначе перехват
  // касанием ставил бы лист не туда, где его видно.
  const шаг40 = stepFling({ x: 300, v: 0 }, 0, 40, TAU_MAX);
  check('положение по времени совпадает с пошаговой формулой',
    Math.abs(flightAt(300, 0, TAU_MAX, 40).x - шаг40.x) < 1e-9, true);
  check('нулевой путь — нулевой полёт',
    JSON.stringify(flightKeyframes(0, 0, TAU_MAX)), JSON.stringify({ duration: 0, xs: [0] }));
  // Между кадрами компоновщик ведёт прямую. Ломаная не должна отходить от
  // экспоненты заметно глазу — меньше пикселя на полном шаге.
  let отход = 0;
  for (const [x0, tau] of [[408, TAU_MAX], [408, TAU_MIN], [120, TAU_MIN]]) {
    const k = flightKeyframes(x0, 0, tau);
    for (let j = 0; j <= 2000; j++) {
      const t = (k.duration * j) / 2000;
      const u = (t / k.duration) * FLIGHT_SAMPLES;
      const i = Math.min(FLIGHT_SAMPLES - 1, Math.floor(u));
      const ломаная = k.xs[i] + (k.xs[i + 1] - k.xs[i]) * (u - i);
      отход = Math.max(отход, Math.abs(ломаная - flightAt(x0, 0, tau, t).x));
    }
  }
  check('ломаная по ключевым кадрам отходит от кривой меньше чем на 1 px', отход < 1, true);
});
